"""Frozen protocol and final-identifier-safe manifest guards for the corrective study."""
...
SCHEMA_VERSION = "realm26-harmonized-replication-v1"
MANIFEST_SCHEMA_VERSION = "realm26-harmonized-manifest-v1"
PROTOCOL_ID = "realm26_harmonized_static_react_v1"
...
def validate_protocol(protocol: Mapping[str, Any]) -> None:
    ...
    if int(protocol.get("seed", -1)) != 20260715:
    ...
    if protocol.get("claims_scope") != "development_only_corrective_study_separate_from_original_gate":
        raise ProtocolError("Claims-scope separation changed")
    if protocol.get("publication") != {
        "base_branch": "realm26/archival-short-paper",
        "branch": "realm26/prompt-context-harmonized-replication",
        "remote": "origin",
    }:
...
def _artifact_paths(protocol: Mapping[str, Any]) -> Dict[str, Path]:
    ...
    return {
        "analysis": finance / "analyze_realm26_harmonized_replication.py",
        "attestation": resolve_path(protocol_path, str(protocol["test_attestation"])),
        "base_methods": finance / "finance_methods.py",
        "executor": finance / "run_realm26_harmonized_replication.py",
        ...
        "methods": finance / "realm26_harmonized_methods.py",
        "data_adapter": finance / "realm26_harmonized_data.py",
        "model_snapshot": resolve_path(protocol_path, str(protocol["model_snapshot"])),
        "prompts": finance / "realm26_harmonized_prompts.py",
        "protocol_memo": finance.parents[2] / "paper" / "realm2026" / "harmonized_replication_protocol.md",
...
def _source_paths(protocol: Mapping[str, Any]) -> Tuple[Path, Path]:
    protocol_path = Path(str(protocol.get("_path", DEFAULT_PROTOCOL_PATH)))
    return (
...
def _identifier_exclusions(protocol: Mapping[str, Any], dataset_id: str) -> Dict[str, Any]:
    ...
    source_protocol_path, second_path = _source_paths(protocol)
    ...
    return {
        "indices": development_indices | final_indices | second_indices,
        "example_ids": development_ids | final_ids | second_ids,
        ...
        "counts": {
            ...
            "total": len(development_indices | final_indices | second_indices),
        },
...
def build_manifest(protocol: Mapping[str, Any], env_factory: Callable[[str], Any]) -> Dict[str, Any]:
    ...
    for dataset_id in DATASETS:
        ...
        if dataset_id == "tatqa":
            ...
            policy = "seeded proportional answer_type x answer_source sample from contexts containing no excluded identifiers"
            ...
            policy = "one seeded turn per new dialogue after identifier-only exclusions"
        else:
            selected = _sample(candidates, per_dataset, seed, f"{PROTOCOL_ID}:{dataset_id}")
            policy = "seeded sample after identifier-only exclusions"
