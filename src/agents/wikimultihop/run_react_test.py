"""
Test Script: Run ReAct on 2WikiMultiHop Dataset

This script runs the ReAct agent on 5 tasks from each task type in the 
2WikiMultiHop train dataset and outputs detailed logs.

Uses the same task indices as the Nexus test for fair comparison.
"""
...
def main():
    ...
    print("=" * 70)
    print("2WikiMultiHop - ReAct Agent Test")
    ...
    for task_type in TASK_TYPES:
        ...
        # Get indices for this task type (same as Nexus test)
        ...
        for i, idx in enumerate(indices):
            ...
            try:
                reward, info = run_react(env, idx, to_print=True)
    ...
    results_file = os.path.join(results_dir, f"react_test_{timestamp}.json")
