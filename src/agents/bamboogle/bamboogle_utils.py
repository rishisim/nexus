"""
Shared utilities for Bamboogle agents.

Bamboogle: Multi-hop compositional QA (free-form answers).
"""
...
def get_bamboogle_env():
    """Get or initialize the Bamboogle environment."""
    global env
    if env is None:
        env = wikienv.WikiEnv()
        env = wrappers.BamboogleWrapper(env, split="test")
        env = wrappers.LoggingWrapper(env)
    return env


# --- Prompt Loading ---
PROMPT_FILE_PATH = './prompts/bamboogle.json'
WEBTHINK_PROMPT_TEMPLATE = ""

try:
    script_dir = os.path.dirname(os.path.abspath(__file__))
    prompt_path = os.path.join(script_dir, 'prompts', 'bamboogle.json')
...
These questions require combining information from multiple Wikipedia articles. Think step by step about what intermediate facts you need.
...
def run_single_trace(idx, initial_prompt_template, to_print=True, temperature=None):
    """Execute a single ReAct trace for a Bamboogle question."""
    bamboogle_env = get_bamboogle_env()

    question = bamboogle_env.reset(idx=idx)
    ...
    for i in range(1, 8):
        ...
        obs, r, done, info = step(bamboogle_env, action_lowercase)
    ...
    if not done:
        ...
        obs_finish, r_finish, done_finish, info_finish = step(bamboogle_env, "finish[null]")
