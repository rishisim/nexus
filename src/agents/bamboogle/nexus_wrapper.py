...
from bamboogle_utils import get_bamboogle_env, llm, llm_judge_answer
...
def run_nexus(idx, prompt_template=None, to_print=True):
    """Run Nexus agent on a single Bamboogle question."""
    if to_print:
        print("="*60)
        print("[FRAMEWORK] Nexus (Scout -> Architect -> Adjudicator)")
        print("="*60)

    env = get_bamboogle_env()
    ...
    agent = NexusAgent(llm_func=llm, env=wrapped_env, task_type="bamboogle")
