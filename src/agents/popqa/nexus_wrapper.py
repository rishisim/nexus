...
from popqa_utils import get_popqa_env, llm, llm_judge_answer
...
def run_nexus(idx, prompt_template=None, to_print=True):
    """Run Nexus agent on a single PopQA question."""
    if to_print:
        print("="*60)
    ...
    agent = NexusAgent(llm_func=llm, env=wrapped_env, task_type="popqa")
