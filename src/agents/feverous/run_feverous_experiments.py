...
from rewoo_wrapper import run_rewoo

class FEVEROUSExperimentRunner:
    ...
    def __init__(
        ...
        results_base_dir: str = "../../../results/feverous",
        seed: int = 42,
        ...
        Args:
            ...
            num_examples: Number of FEVEROUS examples to run
            frameworks: List of frameworks ['react', 'nexus']
            results_base_dir: Base directory for results
            seed: Random seed for reproducibility
            retry_failed: Whether to retry previously failed questions
            specific_indices: List of specific indices to run (overrides num_examples)
        """
        self.model = model
        ...
        self.max_feverous_dev_examples = 7890  # From wrapper loading
        
        # Create seed-based run directory
        ...
        print("="*70, flush=True)
        print(f"[EXPERIMENT] FEVEROUS Agent Evaluation", flush=True)
        print("="*70, flush=True)
    ...
    def _load_or_create_config(self) -> Dict:
        ...
        if self.config_path.exists():
            ...
            config = {
                "seed": self.seed,
                "model": self.model,
                "max_feverous_dev_examples": self.max_feverous_dev_examples,
    ...
    def select_indices(self) -> List[int]:
        ...
        all_indices = list(range(self.max_feverous_dev_examples))
        ...
        print(f"\n[INDICES] Total available: {self.max_feverous_dev_examples}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run FEVEROUS experiments with continuation support")
    ...
    runner = FEVEROUSExperimentRunner(
        model=args.model,
